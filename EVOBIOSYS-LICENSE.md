...
This addendum supplements AMPL 1.0 for organizations that wish to use this
software **without** complying with AMPL 1.0's copyleft and parallel-network
provisions — for example, to modify the software and run it as part of a
proprietary service without open-sourcing those modifications.

**Every user is under a license.** There is no public-domain or
unconditional-use tier. Small users comply with AMPL 1.0 (free, with
copyleft obligations); larger commercial users obtain a separate
commercial license from the Steward. The threshold that separates the
two is defined in Section 2.
...
- **Case-by-Case Adjustment:** The Starting Value is the default, not a
  hard rule. The Steward may agree in writing on a different threshold
  for a particular organization based on sector, mission alignment, and
  stage — e.g. non-profits, research groups, small start-ups, and aligned
  partners typically see higher thresholds than well-funded commercial
  users. To discuss an alternative threshold, write to the Steward
...
- **Adjustment Cadence:** The Steward publishes a revised Resources Threshold
  ...
  track the same cadence from the date of their agreement.
- **Rounding:** Adjusted values round to the nearest EUR 100.
- **Fallback Index:** If Vienna CPI is discontinued, the Steward designates
  a successor index that tracks euro-area consumer inflation in a
  comparable way. If none is designated within 12 months of
  discontinuation, the European Central Bank's Harmonised Index of
...
trademark rights or transfer copyright.

## 5. Good-Faith Assumption

The Steward assumes good faith. Organizations that cross the threshold
mid-year have until the end of that calendar year to either comply with
AMPL 1.0 or obtain a commercial license. There is no retroactive liability
for periods in which the threshold was not met.

## 6. No Other Changes

All other provisions of AMPL 1.0 — patent grants, disclaimer of warranty,
limitation of liability, steward of future versions, parallel-network
extension — continue to apply unchanged.
